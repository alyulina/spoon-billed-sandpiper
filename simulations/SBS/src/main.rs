...
fn main() {
    ...
    let filename8 = format!("SBS_inbread_after{}_{}_{}.txt",                 arg1_str, arg2_str, arg3_str);
    let filename9 = format!("SBS_inbread_after_bneck{}_{}_{}.txt",                 arg1_str, arg2_str, arg3_str);
    ...
    let mut inbread_after          = File::create(&filename8).unwrap();
    let mut inbread_after_bneck    = File::create(&filename9).unwrap();
...
//inbreeding after complex demography

writeln!(inbread_after, "{}\tinbread", runs).expect("Failed to write to outfile");

newpopsize = 2;
for i in 0..100
{

let mut inbread: Vec<HashMap<u32, u8>> = Vec::new();

let mut fitness_inbread = 0.0;

(fitness_inbread) = inbreeding (&mut pop, &selection, &popsize, &mut rng);

writeln!(inbread_after, "{}", fitness_inbread).expect("Failed to write to outfile");
...
//inbreeding after bottleneck

writeln!(inbread_after_bneck, "{}\tinbread", runs).expect("Failed to write to outfile");
...
writeln!(inbread_after_bneck, "{}", fitness_inbread).expect("Failed to write to outfile");
...
fn inbreeding (pop: &mut Vec<HashMap<u32, u8>>, selection: &Vec<(f64, f64)>, popsize: &usize, rng: &mut impl Rng)
 -> (f64)
{
    ...
    while !selected
    {
     ...
     if (mother != father){selected = true;} 
    }

    //here we create two children from the same mother and father
    for i in 0..2
    {
    ...
    //here we mate two children and produce one inbread grandchild
        ...
        if let Some(mother_genome) = newpop.get(mother) //pass the mutations from mother to child
        {
        ...
        if let Some(father_genome) = newpop.get(father) // pass the mutations from father to child
        {
        ...
        let mut fitness_inbreeding = 1.0;
        
        //get fitness of child
         for (gene, &value) in child_genome.iter() 
         {
             if value == 2 
             { fitness_inbreeding *= f64::exp(selection[*gene as usize].0);} // if 11 - s 
          else if value == 1 
             { fitness_inbreeding *= f64::exp(selection[*gene as usize].1);} // 01 - sh
    
             if fitness_inbreeding < 0.0 {fitness_inbreeding = 0.0;}
         }
                    
return (fitness_inbreeding);
}
